...
public class FindIndex {
	...
	public void makeIndex(String path,String table) throws Exception{
		...
		//建索引单独处理
		//查询索引，不存在就新建索引
		String sql = "select *   from sys.user_indexes where table_owner='"+dbutil.getUser().toUpperCase()+"' and table_name='"+table.toUpperCase()+"' and index_type='DOMAIN'";
		Statement statc = conn.createStatement();
		ResultSet rs =  statc.executeQuery(sql);
		int i=0;
		String  index_name = "" ;
		String  domidx_status = "" ;
		String  domidx_opstatus = "" ;
		while(rs.next()){
			index_name = rs.getString("index_name");
			domidx_status = rs.getString("DOMIDX_STATUS");
			domidx_opstatus = rs.getString("DOMIDX_OPSTATUS");
			i++;
		}
		...
		rs.close();
		statc.close();
		if(i<=0||domidx_status.equals("FAILED")||domidx_opstatus.equals("FAILED")){
			fwlog.write("create index "+table.toUpperCase()+"_SX on "+table.toUpperCase()+"(GEOLOC) indextype is MDSYS.SPATIAL_INDEX;\r\n");
			System.out.println("create index "+table.toUpperCase()+"_SX on "+table.toUpperCase()+"(GEOLOC) indextype is MDSYS.SPATIAL_INDEX");
		}
	}
}
